...
const Register = () => {
  ...
  // 1. Cleanup effect to reset cursor if component unmounts
  useEffect(() => {
    setMounted(true);
    return () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    document.body.style.cursor = 'wait'; // Set global wait cursor
  ...
  return (
    <div className={isProcessing ? "cursor-wait" : ""}>
      <div className="min-h-screen flex bg-landingPageLight text-black dark:bg-landingPage dark:text-white">
        ...
        <div className="absolute top-4 left-4">
          <Link 
            ...
            className={`flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-white transition-opacity ${isProcessing ? 'pointer-events-none opacity-50' : ''}`} 
            onMouseEnter={() => !isProcessing && setBacking(true)} 
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <div className="mt-2 relative">
                  ...
                  <button 
                    type="button" 
                    disabled={isProcessing}
                    className={`absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 ${isProcessing ? 'opacity-30' : ''}`} 
              ...
              <div className="flex flex-col items-center space-y-4 pt-2">
                <button 
                  type='submit' 
                  disabled={isProcessing}
                  className={`w-full justify-center rounded-md py-2 px-4 text-sm font-semibold text-white transition-all
                    ${isProcessing ? 'bg-violet-400 opacity-70 pointer-events-none' : 'bg-violet-600 hover:bg-violet-700 active:scale-[0.98]'} 
