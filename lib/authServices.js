...
export class AuthServices {
  ...
  async signup(firstName, lastName, email, password) {
    const { data, error } = await supabase.auth.signUp({
      ...
      password: password,
      options: {
        emailRedirectTo: "https://bolt-7y2c.vercel.app/pages/confirm",
        data: {
          firstName: firstName,
          lastName: lastName,
        },
  ...
  async oAuthSignup() {
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: "https://bolt-7y2c.vercel.app/pages/authConfirm",
      },
    });
  ...
  async sendResetLink(email) {
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: "https://bolt-7y2c.vercel.app/pages/recovery/newPassword",
    });
