...
export class PublicServices {
  ...
  async deleteHistory(chat_id) {
    // Fetch all image URLs before CASCADE deletes them
    ...
    const allImageUrls = messages
      ...
      .flatMap((msg) => msg.images.map((img) => img.image_url));
    ...
    // This will CASCADE delete messages, which will CASCADE delete images
    const { data, error } = await supabase
      .from("history")
      .delete()
      .eq("chat_id", chat_id)
  ...
  async addMessages(chat_id, messages) {
    ...
    console.log(
      "Inserted message before. Chat id & messages",
      chat_id,
      messageRecords,
    );
    // Insert the message
    ...
    console.log("Inserted message after", insertedMessages);
  ...
  async uploadImages(imageUrls) {
    ...
    const uploadPromises = imageUrls.map(async (imageUrl) => {
      // Use proxy only for external URLs
      const fetchUrl = imageUrl.startsWith("blob:")
        ? imageUrl
